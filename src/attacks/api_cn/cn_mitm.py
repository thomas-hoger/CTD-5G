...
class CNMitm:
    ...
    def _nrf_poisonning(nf_to_spoof: str, display=True) -> bool: 
        ...
        infos = CNMitm.attacker_instance.get_nf_info("", nf_to_spoof, display=display)
        ...
        for data in infos["nfInstances"]:
            ...
            removed = legitimate_instance.remove_nf("", display=display)
            CNMitm.removed_instances.append(legitimate_instance)
    ...
    def stop(display=True) -> bool:
        ...
        success = CNMitm.mitm_instance.remove_nf("",display=display)
        
        # refresh the token and remove the nf_instance to avoid polluting the NRF
        success = success and CNMitm.attacker_instance.remove_nf("", display=False)
        
        # Re-add the legitimate NFs
        for instance in CNMitm.removed_instances:
